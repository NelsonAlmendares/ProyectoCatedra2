                ...
                using (var rd = GetEmployees.ExecuteReader())
                {
                    ...
                    while (rd.Read())
                    {
                        ...
                        ObjList.Add(new ModeloPelicula()
                        {
                             ...
                             */
                            ID_Pelicula = Convert.ToInt32(rd["ID"]),
                            Poster = rd["Nombre"].ToString(),
                            Titulo = rd["Apellidos"].ToString(),
                            Sinopsis = rd["Nacimiento"].ToString(),
                            Director = rd["Posicion"].ToString(),
                            Genero = rd["Posicion"].ToString(),
                            Fecha_Estreno = rd["Posicion"].ToString()
                        });
                    }
                }
            }
